            ...
            <div class="p-6 space-y-6">
                @if($order->price['cancellation_fee'] > 0)
                    <div class="flex p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400" role="alert">
                        ...
                        <div>
                            <span class="font-medium">Important!</span> Your package includes a cancellation fee of ${{ number_format($order->price['cancellation_fee'], 2) }} which must be paid to cancel
                        </div>
                    </div>
                @endif
                            ...
                            @if($order->price['cancellation_fee'] > 0)
                                ...
                                @foreach (App\Models\Gateways\Gateway::getActive() as $gateway)
                                    @if($gateway->name == 'Balance') 
                                        <option value="{{ $gateway->id }}" @if(Auth::user()->balance >= $order->price['cancellation_fee']) selected @endif>Pay with Balance (${{ number_format(Auth::user()->balance, 2) }})</option>
                                        @continue
                                    @endif
